...
void ListaLibros::registrarLibro(const Libro &libro)
{
    ...
    std::vector<Libro> librosExistentes = cargarLibrosDesdeCSV("libros.csv");
...
std::vector<Libro> ListaLibros::cargarLibrosDesdeCSV(const std::string& rutaArchivo) const {
...
// Impresión de libros
void ListaLibros::imprimirLibros(const std::string& rutaArchivo) const{
    std::vector<Libro> libros = cargarLibrosDesdeCSV(rutaArchivo);
...
void ListaLibros::modificarLibroEnCSV(const std::string& identificador, const std::string& campoAModificar) {
    ...
    std::vector<Libro> libros = cargarLibrosDesdeCSV("libros.csv");
...
void ListaLibros::ordenarLibrosPorTitulo() {
    ...
    std::cout << "\nLibros ordenados por título:" << std::endl;
    for (const auto& libro : libros) {
        std::cout << "Título: " << libro.getTitulo() << std::endl;
        std::cout << "Autor: " << libro.getAutor().getNombreCompleto() << std::endl;
        std::cout << "ISBN: " << libro.getIsbn() << std::endl;
    ...
    actualizarArchivoCSVOrdenado(libros);
}

void ListaLibros::quicksortLibros(std::vector<Libro>& libros, int inicio, int fin) {
    if (inicio < fin) {
        int indicePivote = particionLibros(libros, inicio, fin);
...
int ListaLibros::particionLibros(std::vector<Libro>& libros, int inicio, int fin) {
    std::string pivote = libros[fin].getTitulo();
    
    int i = inicio - 1;
    
    for (int j = inicio; j < fin; j++) {
        if (libros[j].getTitulo() <= pivote) {
            i++;
            
            std::swap(libros[i], libros[j]);
        }
    }
    
    std::swap(libros[i + 1], libros[fin]);
    
    return i + 1;
}

void ListaLibros::actualizarArchivoCSVOrdenado(const std::vector<Libro>& librosOrdenados) {
    std::ofstream archivoCSV("librosordenados.csv");

    if (!archivoCSV.is_open()) {
        std::cerr << "Error al abrir el archivo CSV para guardar los libros ordenados." << std::endl;
        return;
    }
    ...
    for (const auto& libro : librosOrdenados) {
        ...
        std::string nombreCompleto = libro.getAutor().getNombre() + " " + libro.getAutor().getApellido();
        std::string fechaPublicacion = libro.getFechaPublicacion().getFechaComoString();
        std::string isbn = libro.getIsbn();
        std::string editorial = libro.getEditorial();
    ...
    archivoCSV.close();
    std::cout << "Archivo CSV actualizado con libros ordenados." << std::endl;
}