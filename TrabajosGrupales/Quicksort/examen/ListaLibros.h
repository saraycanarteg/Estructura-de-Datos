...
public:
    ...
    // Métodos auxiliares
    void imprimirLibros(const std::string& rutaArchivo) const;                         // Imprimir la lista de libros
    ...
    vector<Libro> cargarLibrosDesdeCSV(const std::string& rutaArchivo) const ; // Generar archivo CSV
    ...
    void quicksortLibros(std::vector<Libro>& libros, int inicio, int fin);
    int particionLibros(std::vector<Libro>& libros, int inicio, int fin);
    void actualizarArchivoCSVOrdenado(const std::vector<Libro>& librosOrdenados);
