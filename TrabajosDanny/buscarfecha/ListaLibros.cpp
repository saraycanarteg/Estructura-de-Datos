...
void ListaLibros::generarArchivoCSV() const {
    ...
    NodoLibro* actual = cabeza;
    do {
        // Obtener datos usando los métodos de las clases
    ...
    archivoCSV.close();
    std::cout << "\nArchivo CSV actualizado: libros.csv" << std::endl;

    
}

void ListaLibros::buscarLibrosPorFechaIngreso(const string& fechaInicio, const string& fechaFin) const {
    ifstream archivoBackup("libros_backup.txt");
    if (!archivoBackup.is_open()) {
        cout << "No se pudo abrir el archivo de respaldo." << endl;
        return;
    }

    string linea, fechaIngreso, titulo, autor, fechaPublicacion, isbn;
    bool encontrado = false;

    // Lambda para procesar y mostrar el libro
    auto procesarLibro = [&]() {
    ...
    while (getline(archivoBackup, linea)) {
        // Verificar si la línea contiene "Fecha Ingreso:"
        if (linea.substr(0, 13) == "Fecha Ingreso:") {
            fechaIngreso = linea.substr(15); // Extraer la fecha

            // Leer los siguientes datos del libro
            if (getline(archivoBackup, linea)) titulo = linea.substr(8); // Extraer título
            if (getline(archivoBackup, linea)) autor = linea.substr(7); // Extraer autor
            if (getline(archivoBackup, linea)) fechaPublicacion = linea.substr(22); // Extraer fecha publicación
            if (getline(archivoBackup, linea)) isbn = linea.substr(6); // Extraer ISBN

            // Saltar línea de separación
            getline(archivoBackup, linea);
